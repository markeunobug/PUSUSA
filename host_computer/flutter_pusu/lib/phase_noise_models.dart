...
class PhaseNoiseConfig {
  ...
  double get effectiveEnbwHz => effectiveEnbwForRbw(rbwHz);

  double effectiveEnbwForRbw(double pointRbwHz) {
    if (enbwHz != null && (pointRbwHz - rbwHz).abs() < 0.5) {
      return enbwHz!;
    }
    return calculatedEnbwForRbw(pointRbwHz);
  }

  static double calculatedEnbwForRbw(double rbwHz) {
    switch (rbwHz.round()) {
      case 1000:
        return 2137.2954;
      case 10000:
...
class PhaseNoiseDataFrame {
  ...
  /// Protocol v2 uses this field as the current completed sweep number.
  /// Version 1 frames contain the legacy lower-side average result instead.
  ...
  bool get containsRawMeasurement => version >= 2;
...
class PhaseNoiseTrace {
  ...
  factory PhaseNoiseTrace.fromDataFrames(
    ...
    final rawPoints = frameList
        .where((frame) => frame.containsRawMeasurement)
        .map(
    ...
    final legacyAveragePoints = frameList
        .where((frame) => !frame.containsRawMeasurement)
        .map(
          (frame) => PhaseNoisePoint(
            offsetHz: frame.offsetHz.toDouble(),
    ...
    return PhaseNoiseTrace(
      ...
      averagePoints: List<PhaseNoisePoint>.unmodifiable(legacyAveragePoints),
