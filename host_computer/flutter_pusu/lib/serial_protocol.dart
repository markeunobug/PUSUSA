...
class SerialProtocol {
  ...
  static const int _phaseNoiseRawDataVersion = 2;
  ...
  final StreamController<PhaseNoiseStatusFrame> _phaseNoiseStatusStream =
      StreamController.broadcast();
  final StreamController<RealtimeSpectrumFrame> _realtimeSpectrumStream =
      StreamController.broadcast();
  final StreamController<RealtimeSpectrumStatus> _realtimeSpectrumStatusStream =
      StreamController.broadcast();
  ...
  Stream<RealtimeSpectrumFrame> get realtimeSpectrumStream =>
      _realtimeSpectrumStream.stream;
  Stream<RealtimeSpectrumStatus> get realtimeSpectrumStatusStream =>
      _realtimeSpectrumStatusStream.stream;

  void resetReceiveBuffer() {
    _buffer = Uint8List(0);
  }

  /// Discard bytes held by both the OS serial queue and host frame assembler.
  /// Call only after the device accepted STOP and before a new streaming session.
  Future<void> drainAndResetReceiveBuffer() async {
    resetReceiveBuffer();
    ...
    switch (cmd) {
      ...
      case 0x86:
        ...
        print(
          ...
          'sweep=${point.averageIndex} offset=${point.offsetHz}Hz '
      ...
      case 0x87:
        ...
        print(
          ...
          'sweep=${status.averageIndex} offset=${status.currentOffsetHz}Hz '
          'rbw=${status.currentRbwHz}Hz error=${status.errorCode} '
          'warning=${status.warningCode}',
        );
        break;
      case 0x8A:
        final frame = RealtimeSpectrumCodec.parseData(data);
        if (frame == null) {
          badFrameCount++;
          print('Realtime FFT data frame invalid: len=${data.length}');
          break;
        }
        _realtimeSpectrumStream.add(frame);
        break;
      case 0x8B:
        final status = RealtimeSpectrumCodec.parseStatus(data);
        if (status == null) {
          badFrameCount++;
          print('Realtime FFT status frame invalid: len=${data.length}');
          break;
        }
        _realtimeSpectrumStatusStream.add(status);
        break;
      default:
        unknownFrameCount++;
        print(
  ...
  Future<bool> stopSweepConfirmed() {
    return _sendAndWaitAck(0x0A, Uint8List(0));
  }
  ...
  /// Dedicated 35--45 MHz IF / 4096-point real-time FFT protocol (v1).
  void configureRealtimeSpectrum(double centerHz) {
    _manager.sendData(
        _buildFrame(0x18, RealtimeSpectrumCodec.configPayload(centerHz)));
  }

  Future<bool> configureRealtimeSpectrumConfirmed(double centerHz) =>
      _sendAndWaitAck(0x18, RealtimeSpectrumCodec.configPayload(centerHz));

  void startRealtimeSpectrum() =>
      _manager.sendData(_buildFrame(0x19, Uint8List(0)));
  Future<bool> startRealtimeSpectrumConfirmed() =>
      _sendAndWaitAck(0x19, Uint8List(0));
  void stopRealtimeSpectrum() =>
      _manager.sendData(_buildFrame(0x1A, Uint8List(0)));
  Future<bool> stopRealtimeSpectrumConfirmed() =>
      _sendAndWaitAck(0x1A, Uint8List(0));
  void getRealtimeSpectrumStatus() =>
      _manager.sendData(_buildFrame(0x1B, Uint8List(0)));
  Future<bool> getRealtimeSpectrumStatusConfirmed() =>
      _sendAndWaitAck(0x1B, Uint8List(0));
  ...
  PhaseNoiseDataFrame? parsePhaseNoiseData(Uint8List data) {
    ...
    if (data.length != payloadLength ||
        (data[0] != _phaseNoiseVersion &&
            data[0] != _phaseNoiseRawDataVersion)) {
      return null;
    }
  ...
  void dispose() {
    ...
    _realtimeSpectrumStream.close();
    _realtimeSpectrumStatusStream.close();
  }
}
