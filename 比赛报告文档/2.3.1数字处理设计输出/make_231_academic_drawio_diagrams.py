...
OUT_PATH = BASE_DIR / "2.3.1 数字处理设计_学术框图.drawio"
...
class Diagram:
    ...
    def box(self, x, y, w, h, text, fill, stroke, size=16, bold=False, rounded=True):
        style = (
            ...
            "arcSize=6;spacing=12;align=center;verticalAlign=middle;"
    ...
    def group_box(self, x, y, w, h, text, fill="#FFFFFF", stroke=None):
        ...
        style = (
            ...
            "strokeWidth=1.5;dashed=1;dashPattern=8 6;"
    ...
    def edge(self, source, target, label="", stroke=None, dashed=False):
        ...
        style = (
            ...
            f"html=1;endArrow=block;endFill=1;strokeColor={stroke};strokeWidth=1.8;"
...
def page_overall_flow() -> Diagram:
    ...
    for a, b in zip(ids, ids[1:]):
        d.edge(a, b)
    d.group_box(86, 555, 430, 150, "频率规划", COLORS["panel"], COLORS["light_stroke"])
    d.box(130, 615, 340, 58, "LO1 = RF + 2.180 GHz\nLO2 = 2.220 GHz，IF2 = 40 MHz", COLORS["paper"], COLORS["stroke"], size=16)
    d.group_box(635, 555, 430, 150, "扫频密度", COLORS["panel"], COLORS["light_stroke"])
    d.box(682, 615, 335, 58, "内部步进约为 RBW / 2\n最大扫频点数 4096", COLORS["paper"], COLORS["stroke"], size=16)
    d.group_box(1180, 555, 430, 150, "输出方式", COLORS["panel"], COLORS["light_stroke"])
    d.box(1224, 615, 340, 58, "每个频点独立发送\n上位机实时拼接频谱曲线", COLORS["paper"], COLORS["stroke"], size=16)
...
def page_software_arch() -> Diagram:
    ...
    d.box(420, 735, 860, 92, "主循环轮询 device_protocol 与 sweep_engine；扫频活跃时以当前测量点为核心，采样、处理、发送各模块职责清晰，便于调试和扩展。", COLORS["panel"], COLORS["light_stroke"], size=18)
    return d
...
def page_sweep_state() -> Diagram:
    ...
    d.edge(ids[5], ids[6])
    d.edge(ids[6], ids[7])
    d.edge_points(ids[7], ids[1], [(212, 450), (492, 450)], "下一频点", COLORS["line"])
    err = d.box(1300, 375, 250, 150, "异常保护\nLO锁定超时\nDMA超时\n功率测量失败", COLORS["panel2"], COLORS["stroke"], size=17, bold=True)
    d.edge(ids[2], err, dashed=True, stroke=COLORS["soft_line"])
    d.edge(ids[3], err, dashed=True, stroke=COLORS["soft_line"])
    d.edge(ids[5], err, dashed=True, stroke=COLORS["soft_line"])
...
def page_rbw_chain() -> Diagram:
    ...
    for a, b in zip(ids, ids[1:]):
        d.edge(a, b)
    d.group_box(120, 465, 1430, 300, "RBW 档位参数趋势", COLORS["panel"], COLORS["light_stroke"])
    rows = [
    ...
    for i, row in enumerate(rows):
        y = y0 + i * 34
        d.table_cell(x0, y, 160, 34, row[0], COLORS["header"], True)
...
def page_power_calibration() -> Diagram:
    ...
    for a, b in zip(ids, ids[1:]):
        d.edge(a, b)
    cal = d.box(380, 555, 920, 105, "幅度校正项 = 衰减器 - LNA增益 - VGA增益 + 路径校准 + 频率分段校准", COLORS["panel2"], COLORS["stroke"], size=20, bold=True)
    d.edge(ids[3], cal, "raw ADC dBm")
    out = d.box(640, 760, 410, 92, "RF输入端幅度\n用于上位机频谱显示", COLORS["paper"], COLORS["stroke"], size=22, bold=True)
...
def page_rbw_table() -> Diagram:
    ...
    for header, w in zip(headers, col_w):
        d.table_cell(x, y0, w, h, header, COLORS["header"], True)
        x += w
