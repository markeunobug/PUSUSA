...
static void lmx2592_sdio_set_output(void) {
    ...
    GPIO_InitStruct.Pin = LMX2592_SDI_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(LMX2592_SDI_GPIO_Port, &GPIO_InitStruct);
}
...
void LMX2592_SetOutputPower(enum LMX2592_Channel ch, int8_t powerDbm) {
    ...
    if (ch == LMX2592_CHANNEL_A || ch == LMX2592_CHANNEL_A_AND_B) {
        ...
        r46 = (r46 & ~(0x3FU << 8)) | ((uint16_t)(pow_code & 0x3F) << 8);
        LMX2592_WriteRegister(46, r46);
    }
    if (ch == LMX2592_CHANNEL_B || ch == LMX2592_CHANNEL_A_AND_B) {
        uint16_t r47 = lmx2592_shadow[47];
        r47 = (r47 & ~0x003FU) | (pow_code & 0x3F);
        LMX2592_WriteRegister(47, r47);
    }

    // 功率码更新后短暂脉冲对应通道的 PD 位（让输出缓冲器重新锁存功率值）
    {
        uint16_t r46_orig = lmx2592_shadow[46];
        uint16_t r47_orig = lmx2592_shadow[47];   // r47 只用于 MUX/OUTB_POW，不再碰 PD
        uint16_t r46_pd = r46_orig;

        if (ch == LMX2592_CHANNEL_A || ch == LMX2592_CHANNEL_A_AND_B) {
            r46_pd |= (1U << 6);   // OUTA_PD=1
        }
        if (ch == LMX2592_CHANNEL_B || ch == LMX2592_CHANNEL_A_AND_B) {
            r46_pd |= (1U << 7);   // OUTB_PD=1     ← 修正：改用 r46[7]
        }

        if (r46_pd != r46_orig) {
            LMX2592_WriteRegister(46, r46_pd);
        }
        lmx2592_delay_us(30);
        if (r46_pd != r46_orig) {
            LMX2592_WriteRegister(46, r46_orig);
        }
    }
}
...
void LMX2592_SetOutputChannel(enum LMX2592_Channel channel) {
    ...
    switch (channel) {
        case LMX2592_CHANNEL_A:
            r46 &= ~(1U << 6);   // OUTA_PD = 0
            r46 |=  (1U << 7);   // OUTB_PD = 1     ← 修正：改用 r46[7]
            r31 &= ~(1U << 9);   // VCO_DISTA_PD = 0
            r31 |=  (1U << 10);  // VCO_DISTB_PD = 1
            r36 |=  (1U << 10);  // CHDIV_DISTA_EN = 1
            r36 &= ~(1U << 11);  // CHDIV_DISTB_EN = 0
            break;

        case LMX2592_CHANNEL_B:
            r46 |=  (1U << 6);   // OUTA_PD = 1
            r46 &= ~(1U << 7);   // OUTB_PD = 0     ← 修正：改用 r46[7]
            r31 |=  (1U << 9);   // VCO_DISTA_PD = 1
            r31 &= ~(1U << 10);  // VCO_DISTB_PD = 0
            r36 &= ~(1U << 10);  // CHDIV_DISTA_EN = 0
            r36 |=  (1U << 11);  // CHDIV_DISTB_EN = 1
            break;

        case LMX2592_CHANNEL_A_AND_B:
            r46 &= ~((1U << 6) | (1U << 7));  // OUTA_PD=0, OUTB_PD=0
            r31 &= ~((1U << 9) | (1U << 10)); // 两个 VCO 分布 buffer 都开启
            r36 |=  ((1U << 10) | (1U << 11)); // 两个 CHDIV 分布 buffer 都开启
            break;
    }

    LMX2592_WriteRegister(46, r46);
    LMX2592_WriteRegister(47, r47);  // MUX 和 OUTB_POW 仍在 r47，保持不变
