...
static u32       current_sg_dma_samples = FFT_SIZE;
static float32_t current_decimated_sample_rate_hz = ADC_SAMPLE_RATE_HZ;
static signal_processing_peak_debug_t peak_debug = {.version = 1U};
static signal_processing_peak_debug_t last_peak_debug = {.version = 1U};
...
u32 signal_processing_get_sg_dma_samples(void)
{
    return current_sg_dma_samples;
}
...
static void sweep_configure_for_rbw(rbw_mode_t mode)
{
     ...
     */
    {
        u32 required_blocks_per_transfer = 1U;
        u32 simple_blocks_per_transfer;
        u32 sg_blocks_per_transfer;
        ...
        if (decimated_per_block > 0U) {
            required_blocks_per_transfer =
                ((u32)accum_target + decimated_per_block - 1U) /
                decimated_per_block;
        }
        if (required_blocks_per_transfer < 1U) {
            required_blocks_per_transfer = 1U;
        }

        simple_blocks_per_transfer = required_blocks_per_transfer;
        if (simple_blocks_per_transfer > DMA_SWEEP_MAX_BLOCKS_PER_TRANSFER) {
            simple_blocks_per_transfer = DMA_SWEEP_MAX_BLOCKS_PER_TRANSFER;
        }
        current_dma_samples = simple_blocks_per_transfer * FFT_SIZE;

        sg_blocks_per_transfer = required_blocks_per_transfer;
        if (sg_blocks_per_transfer > DMA_SWEEP_SG_MAX_BLOCKS_PER_TRANSFER) {
            sg_blocks_per_transfer = DMA_SWEEP_SG_MAX_BLOCKS_PER_TRANSFER;
        }
        current_sg_dma_samples = sg_blocks_per_transfer * FFT_SIZE;
    }
