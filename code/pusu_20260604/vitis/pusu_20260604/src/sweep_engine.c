...
/* PL IQ output can be as low as 100 kSPS. A 4096-word DMA frame then needs
 * roughly 41 ms after the S2MM channel is armed, and the AXIS FIFO may only
 * contain a partial frame after a DMA reset. Keep this timeout comfortably
 * above that low-rate fill time.
 */
#define SWEEP_ENGINE_FRAME_TIMEOUT_LOOPS  50000000U
/* The fixed-LO residual sweep path is only valid when the whole span fits inside
 * the decimated PL IQ sample rate. Keep it disabled for normal spectrum sweeps:
 * each point should tune LO1 to current_rf_hz and measure near DC.
 */
#define SWEEP_ENGINE_USE_FIXED_LO_RESIDUAL_SWEEP 0

static void sweep_engine_set_error(sweep_engine_t *engine, int error);
static int sweep_engine_start_dma_transfer(sweep_engine_t *engine);
static void sweep_engine_debug_begin(const sweep_engine_t *engine, u32 dma_bytes);
static void sweep_engine_debug_update(const sweep_engine_t *engine, int error);
static uint64_t sweep_engine_tune_rf_hz(const sweep_engine_t *engine);
static float sweep_engine_residual_hz(const sweep_engine_t *engine);
static int g_last_error = SWEEP_ENGINE_OK;
static u8 g_sweep_dbg_frame_ready_seen = 0U;
static u8 g_sweep_dbg_dma_error_seen = 0U;
static u32 g_sweep_dbg_dma_start_status = 0U;
static u32 g_sweep_dbg_dma_bytes = 0U;
static u32 g_sweep_dbg_pl_output_at_arm = 0U;
...
int sweep_engine_poll(sweep_engine_t *engine)
{
    ...
    case SWEEP_ENGINE_STATE_PREPARE:
        ...
        if (pusu_pl_dsp_configure_rbw(engine->plan.rbw_mode) != XST_SUCCESS) {
            sweep_engine_set_error(engine, SWEEP_ENGINE_ERR_PL_DSP_CONFIG);
            return SWEEP_ENGINE_ERR_PL_DSP_CONFIG;
        }
    ...
    case SWEEP_ENGINE_STATE_SET_LO1:
        ...
        signal_processing_set_debug_point_index(engine->current_point);
        signal_processing_set_residual_frequency_hz(sweep_engine_residual_hz(engine));
        sweep_profile_begin(SWEEP_PROFILE_SECTION_POINT_TOTAL);
        sweep_profile_begin(SWEEP_PROFILE_SECTION_SET_LO1);
        if (lo_control_set_lo1_for_rf_hz(sweep_engine_tune_rf_hz(engine)) != XST_SUCCESS) {
    ...
    case SWEEP_ENGINE_STATE_ARM_DMA:
        return sweep_engine_start_dma_transfer(engine);
    ...
    case SWEEP_ENGINE_STATE_WAIT_FRAME:
        if (dma_capture_take_error() != 0) {
            g_sweep_dbg_dma_error_seen = 1U;
            sweep_engine_debug_update(engine, SWEEP_ENGINE_ERR_FRAME_TIMEOUT);
        ...
        if (dma_capture_frame_ready() != 0) {
            g_sweep_dbg_frame_ready_seen = 1U;
            sweep_profile_end(SWEEP_PROFILE_SECTION_DMA_WAIT);
            ...
            sweep_engine_debug_update(engine, SWEEP_ENGINE_OK);
...
static int sweep_engine_start_dma_transfer(sweep_engine_t *engine)
{
    u32 dma_bytes = dma_capture_bytes_for_samples(
        signal_processing_get_dma_samples());

    sweep_engine_debug_begin(engine, dma_bytes);

    if (pusu_pl_dsp_prepare_clean_dma_start(engine->plan.rbw_mode) !=
        XST_SUCCESS) {
        g_sweep_dbg_dma_start_status = XST_FAILURE;
        sweep_engine_debug_update(engine, SWEEP_ENGINE_ERR_PL_DSP_CONFIG);
        sweep_profile_end(SWEEP_PROFILE_SECTION_POINT_TOTAL);
        sweep_engine_set_error(engine, SWEEP_ENGINE_ERR_PL_DSP_CONFIG);
        return SWEEP_ENGINE_ERR_PL_DSP_CONFIG;
    }
    ...
    g_sweep_dbg_dma_start_status = (u32)dma_capture_start(dma_bytes);
    if (g_sweep_dbg_dma_start_status != (u32)XST_SUCCESS) {
        sweep_engine_debug_update(engine, SWEEP_ENGINE_ERR_FRAME_TIMEOUT);
    ...
    if (pusu_pl_dsp_release_clean_dma_start() != XST_SUCCESS) {
        sweep_engine_debug_update(engine, SWEEP_ENGINE_ERR_FRAME_TIMEOUT);
        sweep_profile_end(SWEEP_PROFILE_SECTION_DMA_START);
        sweep_profile_end(SWEEP_PROFILE_SECTION_POINT_TOTAL);
        sweep_engine_set_error(engine, SWEEP_ENGINE_ERR_FRAME_TIMEOUT);
...
static void sweep_engine_debug_begin(const sweep_engine_t *engine, u32 dma_bytes)
{
    ...
    g_sweep_dbg_frame_ready_seen = 0U;
    g_sweep_dbg_dma_error_seen = 0U;
    g_sweep_dbg_dma_start_status = 0U;
    g_sweep_dbg_dma_bytes = dma_bytes;
    g_sweep_dbg_pl_output_at_arm = 0U;

    if (pusu_pl_dsp_read_status(&pl_status) == XST_SUCCESS) {
        g_sweep_dbg_pl_output_at_arm = pl_status.output_sample_count;
    }
...
static void sweep_engine_debug_update(const sweep_engine_t *engine, int error)
{
    ...
    pusu_pl_dsp_status_t pl_status;
    u32 pl_output_now = 0U;

    dma_capture_get_diag(&dma_diag);
    if (pusu_pl_dsp_read_status(&pl_status) == XST_SUCCESS) {
        pl_output_now = pl_status.output_sample_count;
    }

    signal_processing_set_sweep_debug(
        (engine == 0) ? 0U : (u8)engine->state,
        error,
        g_sweep_dbg_frame_ready_seen,
        g_sweep_dbg_dma_error_seen,
        (engine == 0) ? 0U : engine->wait_counter,
        g_sweep_dbg_dma_start_status,
        dma_diag.s2mm_dmasr,
        dma_diag.last_irq_status,
        g_sweep_dbg_pl_output_at_arm,
        pl_output_now,
        signal_processing_get_accum_count(),
        signal_processing_get_accum_target(),
        g_sweep_dbg_dma_bytes);
}

static uint64_t sweep_engine_tune_rf_hz(const sweep_engine_t *engine)
{
#if PL_DSP_DEBUG_IQ_PATH && SWEEP_ENGINE_USE_FIXED_LO_RESIDUAL_SWEEP
    if (engine != 0 && engine->plan.point_count > 1U) {
        return (engine->plan.start_hz + engine->plan.stop_hz) / 2ULL;
    }
#endif
    return (engine == 0) ? 0ULL : engine->current_rf_hz;
}
...
#if PL_DSP_DEBUG_IQ_PATH && SWEEP_ENGINE_USE_FIXED_LO_RESIDUAL_SWEEP
    if (engine != 0 && engine->plan.point_count > 1U) {
        uint64_t center_hz = (engine->plan.start_hz + engine->plan.stop_hz) / 2ULL;
        return (float)((int64_t)engine->current_rf_hz - (int64_t)center_hz);
    }
#endif
    (void)engine;
    return 0.0f;
