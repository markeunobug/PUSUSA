...
/* Match sweep_engine.c: the PL IQ stream bottoms out at 100 kSPS, so one
 * 4096-word S2MM frame can take about 41 ms to arrive after arming DMA.
 */
#define PN_FRAME_TIMEOUT_LOOPS  50000000U
...
int phase_noise_engine_poll(phase_noise_engine_t *engine)
{
    ...
    case PN_STEP_PREPARE_CARRIER:
    {
        ...
        if (pusu_pl_dsp_configure_rbw(search_rbw_mode) != XST_SUCCESS) {
    ...
    case PN_STEP_REARM_DMA:
        ...
        if (dma_capture_start(dma_capture_bytes_for_samples(
                signal_processing_get_dma_samples())) != XST_SUCCESS) {
...
static int start_current_point(phase_noise_engine_t *engine)
{
    ...
    if (engine->state == PN_STATE_SEARCHING_CARRIER) {
        ...
        if (pusu_pl_dsp_configure_rbw(point->rbw_mode) != XST_SUCCESS) {
            set_error(engine, PN_ERR_INTERNAL);
            return -1;
        }
