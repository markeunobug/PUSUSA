...
static float32_t residual_freq_hz;
static float32_t residual_nco_phase;
...
#if PL_DSP_DEBUG_IQ_PATH
static void pl_debug_iq_unpack_to_time_domain(volatile u16 *rx_buffer);
static void pl_debug_apply_residual_nco(void);
#endif
#if RBW_10K_SYNTH_DDC_MODE != RBW_10K_SYNTH_DDC_MODE_REAL_ADC
static void ddc_generate_synthetic_10k_block(void);
#endif
static void sweep_configure_for_rbw(rbw_mode_t mode);
static void update_pl_peak_debug(const pusu_pl_dsp_status_t *pl_status);
...
static void ddc_debug_reset(void)
{
    ...
    float32_t debug_sample_rate_hz = current_decimated_sample_rate_hz;
    int i;

    if (debug_sample_rate_hz <= 0.0f) {
        debug_sample_rate_hz = ADC_SAMPLE_RATE_HZ;
    }

    ddc_debug_power_sum = 0.0f;
    ddc_debug_sample_count = 0U;
    for (i = 0; i < 5; i++) {
        float32_t step = 2.0f * PI * freqs_hz[i] / debug_sample_rate_hz;
...
{
    residual_freq_hz = freq_hz;
    residual_nco_phase = 0.0f;
}
...
void signal_processing_accumulate_dma(volatile u16 *rx_buffer, u32 dma_samples)
{
    ...
    for (offset = 0; offset < dma_samples; offset += FFT_SIZE) {
        ...
        pl_debug_iq_unpack_to_time_domain(rx_buffer + (offset * 2U));
        pl_debug_apply_residual_nco();
        ...
        copy_count = (int)FFT_SIZE;
        if (copy_count > ((int)ACCUM_BUFFER_SIZE - accum_count)) {
            copy_count = (int)ACCUM_BUFFER_SIZE - accum_count;
        }
        if (copy_count > 0) {
            memcpy(accum_i + accum_count, time_domain_real,
                   (size_t)copy_count * sizeof(float32_t));
            memcpy(accum_q + accum_count, time_domain_imag,
                   (size_t)copy_count * sizeof(float32_t));
            produced = copy_count;
        } else {
            produced = 0;
...
#if RBW_10K_USE_FIR_DECIMATOR
        if (current_rbw_mode == RBW_MODE_10K) {
#if RBW_10K_RESET_FIR_DECIMATOR_EACH_DMA_BLOCK
...
{
    return (u32)current_decimated_sample_rate_hz;
}

u32 signal_processing_get_current_decimation(void)
{
    return pusu_pl_dsp_get_active_decimation();
}
...
{
    return (accum_count < 0) ? 0U : (u32)accum_count;
}

u32 signal_processing_get_accum_target(void)
{
    return (accum_target < 0) ? 0U : (u32)accum_target;
}
...
void signal_processing_set_sweep_debug(u8 state,
                                       ...
                                       u32 accum_count_value,
                                       u32 accum_target_value,
                                       u32 dma_bytes)
{
    peak_debug.sweep_state = state;
    peak_debug.sweep_error_code =
        (u8)((error < 0) ? (-error) : error);
    peak_debug.sweep_frame_ready_seen = frame_ready_seen;
    peak_debug.sweep_dma_error_seen = dma_error_seen;
    peak_debug.sweep_wait_counter = wait_counter;
    peak_debug.sweep_dma_start_status = dma_start_status;
    peak_debug.sweep_dma_dmasr = dma_dmasr;
    peak_debug.sweep_dma_last_irq = dma_last_irq;
    peak_debug.sweep_pl_output_count_at_arm = pl_output_count_at_arm;
    peak_debug.sweep_pl_output_count_now = pl_output_count_now;
    peak_debug.sweep_accum_count = accum_count_value;
    peak_debug.sweep_accum_target = accum_target_value;
    peak_debug.sweep_dma_bytes = dma_bytes;
}

void signal_processing_get_peak_debug(signal_processing_peak_debug_t *out_debug)
{
    if (out_debug != 0) {
        signal_processing_peak_debug_t dbg = (peak_debug_valid != 0U)
            ? last_peak_debug
            : peak_debug;
        dbg.sweep_state = peak_debug.sweep_state;
        dbg.sweep_error_code = peak_debug.sweep_error_code;
        dbg.sweep_frame_ready_seen = peak_debug.sweep_frame_ready_seen;
        dbg.sweep_dma_error_seen = peak_debug.sweep_dma_error_seen;
        dbg.sweep_wait_counter = peak_debug.sweep_wait_counter;
        dbg.sweep_dma_start_status = peak_debug.sweep_dma_start_status;
        dbg.sweep_dma_dmasr = peak_debug.sweep_dma_dmasr;
        dbg.sweep_dma_last_irq = peak_debug.sweep_dma_last_irq;
        dbg.sweep_pl_output_count_at_arm =
            peak_debug.sweep_pl_output_count_at_arm;
        dbg.sweep_pl_output_count_now = peak_debug.sweep_pl_output_count_now;
        dbg.sweep_accum_count = peak_debug.sweep_accum_count;
        dbg.sweep_accum_target = peak_debug.sweep_accum_target;
        dbg.sweep_dma_bytes = peak_debug.sweep_dma_bytes;
        *out_debug = dbg;
    }
}
...
#if PL_DSP_DEBUG_IQ_PATH
    const pusu_pl_dsp_rbw_config_t *pl_cfg = pusu_pl_dsp_get_rbw_config(mode);
    const pusu_pl_dsp_status_t *pl_status = pusu_pl_dsp_get_last_status();
    u32 pl_sample_rate_hz = pusu_pl_dsp_get_active_sample_rate_hz();
#endif
...
#if PL_DSP_DEBUG_IQ_PATH
    comp_fir_skip = (int)pl_cfg->skip_points;
    accum_target = (int)pl_cfg->accum_target;
    if (pl_sample_rate_hz == 0U) {
        pl_sample_rate_hz = pl_cfg->expected_sample_rate_hz;
    }
    current_decimated_sample_rate_hz = (float32_t)pl_sample_rate_hz;
    update_pl_peak_debug(pl_status);
#else
...
static void update_pl_peak_debug(const pusu_pl_dsp_status_t *pl_status)
{
    ...
    peak_debug.pl_available = pl_status->available;
    peak_debug.pl_mode_value = pl_status->pl_mode_value;
    peak_debug.pl_total_decimation = (u16)(pl_status->decimation_ratio & 0xFFFFU);
    peak_debug.pl_sample_rate_hz = pl_status->sample_rate_hz;
    peak_debug.pl_data_format = pl_status->data_format;
    peak_debug.pl_status = pl_status->status;
}
...
static void pl_debug_iq_unpack_to_time_domain(volatile u16 *rx_buffer)
{
    ...
    for (i = 0; i < FFT_SIZE; i++) {
        uint32_t word_index = 2U * i;
        int16_t sample_i = (int16_t)rx_buffer[word_index + 1U];
        int16_t sample_q = (int16_t)rx_buffer[word_index];

        time_domain_real[i] = (float32_t)sample_i / 32768.0f;
        time_domain_imag[i] = (float32_t)sample_q / 32768.0f;
    }
}
static void pl_debug_apply_residual_nco(void)
{
    ...
    if (residual_freq_hz == 0.0f ||
        current_decimated_sample_rate_hz <= 0.0f) {
        return;
    }

    phase = residual_nco_phase;
    phase_step = -2.0f * PI * residual_freq_hz /
        current_decimated_sample_rate_hz;
    ...
    for (i = 0; i < FFT_SIZE; i++) {
        ...
        time_domain_real[i] = (sample_i * c) - (sample_q * s);
        time_domain_imag[i] = (sample_i * s) + (sample_q * c);
