...
#define STATUS_PAYLOAD_SIZE     103U
...
static void send_status(void)
{
    ...
    payload[59] = status.pl_available;
    payload[60] = status.pl_mode_value;
    payload[61] = status.pl_output_select;
    payload[62] = status.pl_reserved;
    write_u32_be(&payload[63], status.pl_status);
    write_u32_be(&payload[67], status.pl_sample_rate_hz);
    write_u32_be(&payload[71], status.pl_decimation);
    write_u32_be(&payload[75], status.pl_frame_words);
    write_u32_be(&payload[79], status.pl_data_format);
    write_u32_be(&payload[83], status.pl_version);
    write_u32_be(&payload[87], status.pl_input_sample_count);
    write_u32_be(&payload[91], status.pl_output_sample_count);
    write_u32_be(&payload[95], status.pl_output_drop_count);
    write_u32_be(&payload[99], status.pl_error_code);
                                                ...
                                                unsigned short *io_len)
{
    ...
    unsigned int offset;
    const unsigned int ext_len = 112U;
    ...
    offset += 4U;
    dst[offset++] = dbg.sweep_state;
    dst[offset++] = dbg.sweep_error_code;
    dst[offset++] = dbg.sweep_frame_ready_seen;
    dst[offset++] = dbg.sweep_dma_error_seen;
    write_u32_be(&dst[offset], dbg.sweep_wait_counter);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_dma_start_status);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_dma_dmasr);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_dma_last_irq);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_pl_output_count_at_arm);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_pl_output_count_now);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_accum_count);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_accum_target);
    offset += 4U;
    write_u32_be(&dst[offset], dbg.sweep_dma_bytes);
    offset += 4U;
