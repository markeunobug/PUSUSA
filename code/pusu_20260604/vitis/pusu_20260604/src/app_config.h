...
/* PL DDC/decimation debug path.
 * 0: legacy AXI DMA stream carries signed 16-bit raw ADC samples.
 * 1: AXI DMA stream carries signed 16-bit I/Q packed into one 32-bit word:
 *    bits[31:16] = I, bits[15:0] = Q.  This path assumes PL already mixed and
 *    decimated the stream, so PS skips the old DDC/CIC stage and only keeps the
 *    post-decimation accumulation, compensating FIR, and power measurement path.
 */
#ifndef PL_DSP_DEBUG_IQ_PATH
#define PL_DSP_DEBUG_IQ_PATH        1
#endif
#define PL_DSP_DEBUG_IQ_FALLBACK_SAMPLE_RATE_HZ 1000000.0f

#if PL_DSP_DEBUG_IQ_PATH
#define DMA_STREAM_BYTES_PER_SAMPLE 4U
#else
#define DMA_STREAM_BYTES_PER_SAMPLE 2U
#endif

#define TRANSFER_LENGTH     (FFT_SIZE * DMA_STREAM_BYTES_PER_SAMPLE)
#define DMA_MAX_SAMPLES     65536U
#define DMA_MAX_BYTES       (DMA_MAX_SAMPLES * DMA_STREAM_BYTES_PER_SAMPLE)
/* AXI DMA S2MM length width is 16 bits in the exported BSP, so one simple
 * transfer must stay below 65536 bytes. Seven legacy ADC FFT blocks is the
 * largest safe block-aligned sweep transfer: 7 * 4096 samples * 2 bytes =
 * 57344 bytes. */
...
/* Legacy PS-side 10K decimator diagnostic.  The active PL IQ path already
 * outputs the selected decimated sample rate, so keep this disabled here. */
...
/* Scheme A validation: real ADC/DDC data, but do not carry the long 10K FIR
 * decimator history across DMA blocks.  This is a diagnostic step; if it
 * improves 40.100 MHz rejection, the final fix should move to a continuous
 * stream design rather than relying on this as the production architecture. */
#define RBW_10K_RESET_FIR_DECIMATOR_EACH_DMA_BLOCK 1
